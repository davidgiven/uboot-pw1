...
#define I2C_ADDR_ALS                       0x4A // bus 0
...
int check_als (void)
{
    ...
    bus = i2c_get_bus_num();
    i2c_set_bus_num(0);
    ret = i2c_read(I2C_ADDR_ALS, 0x02, 1, &reg, 1); //Configuration Reg 0x02
    if (ret)
    {
        printf("is missing\n");
    }
...
int inventory_post_test (int flags)
{
    const char *rev;
    int ret   = 0;
    int index = INDEX_PRIOR_CELESTE_EVT2; //0
    ...
    else if (BOARD_IS_TEQUILA(rev))  // Only on BOARD_ID_TEQUILA boards
    {
        ret  = check_battery();  // Battery may not be installed at SMT station
        ret |= check_power();
        ret |= check_papyrus(index);
    ...
    else if (BOARD_IS_WHITNEY(rev))  // Only on Whitney WAN boards
    {
    ...
    else if (BOARD_IS_WHITNEY_WFO(rev))  // Only on Whitney WFO boards
    {
        ret  = check_proximity();   // Proximity sensor is still being debated
        ret  = check_touch(index);       // Neonode are not preprogrammed
        ret  = check_battery();     // Battery may not be installed at SMT station
    ...
    else if (BOARD_IS_YOSHIME(rev))  // Only on yoshime board
    {
        index = INDEX_AFTER_CELESTE_EVT2;
        ret  = check_touch(index);    // Cypress are not preprogrammed
        ret  = check_battery();       // Battery may not be installed at SMT station
        ret |=check_papyrus(index);  // papyrus i2c address changed but board id unchanged, have to bypass to support both
        ret |= check_power();
        ret |= check_usbmux();
    }

    else if (BOARD_IS_YOSHIME_3(rev))  // Only on yoshime3 board
    {
        index = INDEX_AFTER_CELESTE_EVT2;
        ret  = check_touch(index);    // Cypress are not preprogrammed
        ret  = check_battery();       // Battery may not be installed at SMT station
    ...
    else if (BOARD_IS_CELESTE(rev))  // Only on Celeste WAN boards
    {
        index = INDEX_AFTER_CELESTE_EVT2;
        ret  = check_touch(index);    // Cypress are not preprogrammed
        ret  = check_battery();       // Battery may not be installed at SMT station
        ret |= check_power();
        if (!BOARD_IS_CELESTE_256(rev) && !BOARD_REV_GREATER(rev, BOARD_ID_CELESTE_EVT1_2))
          index = INDEX_PRIOR_CELESTE_EVT2;
        ret |= check_papyrus(index);  // Only on Celeste boards after EVT1.2
        ret |= check_usbmux();
    }
   
    else if (BOARD_IS_CELESTE_WFO(rev))  // Only on Celeste WFO boards
    {
        index = INDEX_AFTER_CELESTE_EVT2;
        ret  = check_touch(index);  // Cypress are not preprogrammed
        ret  = check_battery();     // Battery may not be installed at SMT station
        ret |= check_power();
        if (!BOARD_IS_CELESTE_WFO_512(rev) && !BOARD_REV_GREATER(rev, BOARD_ID_CELESTE_WFO_EVT1_2))
          index = INDEX_PRIOR_CELESTE_EVT2;
        ret |= check_papyrus(index);  // Only on Celeste WFO boards after EVT1.2
        ret |= check_usbmux();
    }
