...
#define LPM_LAB126_SUSPEND_CFG_DEFAULT_DISPLAY_PG             0
#define LPM_LAB126_SUSPEND_CFG_DEFAULT_USE_RC_OSC             1
...
void lpm_disable_unused_clocks(void)
{
	...
	reg32_write(CCM_BASE_ADDR + CCM_CCGR3_OFFSET, 0x11700000);
...
{
	// reduce OSC current
	reg32_write(ANATOP_BASE_ADDR + 0x150,reg32_read(ANATOP_BASE_ADDR + 0x150) | 0x0000C000);
...
void lpm_enter_exit(void)
{
	...
	if (WEAK_2P5_EN)
		reg32_write(ANATOP_BASE_ADDR + 0x130, reg32_read(ANATOP_BASE_ADDR + 0x130) | 0x60000);
	else
		reg32_write(ANATOP_BASE_ADDR + 0x130, reg32_read(ANATOP_BASE_ADDR + 0x130) & (~0x60000));
	...
	// Apply low power setting for analog
	if (WEAK_2P5_EN) {
	...
	if (ARM_POWER_GATE_EN) {
		if (BUS_CLOCK_FREQ == 1)
			...
			mx6q_suspend(3,SUSPEND_OCRAM_PADDR,SUSPEND_OCRAM_BASE);
	} else {
		if(BUS_CLOCK_FREQ == 1)
			mx6q_suspend(2,SUSPEND_OCRAM_PADDR,SUSPEND_OCRAM_BASE);
		else
			mx6q_suspend(1,SUSPEND_OCRAM_PADDR,SUSPEND_OCRAM_BASE);
	}
}
