...
{
	int i = 0; 
	int start_intensity = WARIO_FL_LEVEL0, end_intensity = WARIO_FL_LEVEL12_MID;
	int hop = WARIO_FL_LO_GRP_HOP_LEVEL; 			/* FL intensity skip level */
	int delay_us = WARIO_FL_LO_GRP_DELAY_US;		/* delay between FL intensity updates */

	for (i = start_intensity; i <= end_intensity; i += hop) {
		pmic_fl_set(i);  
		udelay(delay_us);
		if (i == WARIO_FL_LO_TRANSITION_LEVEL) {
			hop = WARIO_FL_MED_GRP_HOP_LEVEL;
		}
	}
}
...
static void setup_regulators(void)
{
	...
	/* Set LDO 2 to the proper voltage for the USB PHY (3V), but disable it */
	pmic_write_reg(PM_GPIO_SADDR, LDO_L02_CNFG1_REG, 0x2c);
	if (BOARD_REV_GREATER(rev, BOARD_ID_WARIO_2)||
	    BOARD_IS_PINOT(rev) ||
	    BOARD_REV_GREATER(rev, BOARD_ID_PINOT_WFO_2GB_EVT1) ||
	    BOARD_REV_GREATER(rev, BOARD_ID_PINOT_WFO_EVT1)) {
	...
	if (BOARD_REV_GREATER(rev, BOARD_ID_PINOT_EVT1_2) ||
		  BOARD_REV_GREATER(rev, BOARD_ID_PINOT_2GB_EVT1_2) ||
		  BOARD_REV_GREATER(rev, BOARD_ID_PINOT_WFO_2GB_EVT1_2) ||
	    BOARD_REV_GREATER(rev, BOARD_ID_PINOT_WFO_EVT1_2)) {
