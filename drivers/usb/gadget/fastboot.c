...
extern int mmc_erase(struct mmc *mmc, unsigned int start_addr, unsigned int end_addr);
/*
 * Serial number
 */
...
static void fastboot_parse_cmd(char *cmdbuf) 
{
    ...
    else if (strncmp(cmdbuf, "erase", 5) == 0) {
	int dot = 0;
	...
	// write 0xFF to erase
	memset(transfer_buffer, 0xFF, BUFFER_LEN);

	src = (unsigned char *) transfer_buffer;
	len = part_size;
	dest = flash_addr;
