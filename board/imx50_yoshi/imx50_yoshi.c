...
#ifdef CONFIG_IRAM_BOOT
		/* Samsung 512 MB chips use 1 CS, all other parts use 2 CS */
...
int board_init(void)
{
#if defined(CONFIG_BIST) && defined(CONFIG_USB_DEVICE)
	/* Detach from the USB bus in case we came from the mfgtool */
	udc_disconnect();
#endif
	/* boot device */
	setup_boot_device();
